...
echo -e "\n${BLUE}[1/7] Ensuring Git credential helper is active...${NC}"
git config credential.helper store
echo -e "${GREEN}✔ Git credential helper is set to store credentials.${NC}"

# 3. Regenerate PDF Manual
echo -e "\n${BLUE}[2/7] Regenerating PDF User Manual from manual.html...${NC}"
...
echo -e "\n${BLUE}[3/7] Syncing web assets and compiling Android APK...${NC}"
bash build-app.sh
echo -e "${GREEN}✔ Android APK built successfully.${NC}"

# 5. Check and install on connected ADB device
echo -e "\n${BLUE}[4/7] Checking for connected Android devices via ADB...${NC}"
...
echo -e "\n${BLUE}[5/7] Deploying frontend assets to Firebase Hosting...${NC}"
firebase deploy --only hosting
echo -e "${GREEN}✔ Firebase Hosting deployment completed.${NC}"

# 7. Deploy Cloudflare Worker
echo -e "\n${BLUE}[6/7] Deploying Cloudflare Worker for AI Chatbot...${NC}"
cd attendify-support-worker
npx wrangler deploy
cd ..
echo -e "${GREEN}✔ Cloudflare Worker deployed.${NC}"

# 8. Sync with Git Repository
echo -e "\n${BLUE}[7/7] Staging, committing, and pushing changes to GitHub...${NC}"
git add .
